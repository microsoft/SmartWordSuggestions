...
def load_ppdb():
    ...
    with open('../../../data/sws_ds/ppdb-2.0-s-lexical', 'r') as f:
        lines = f.readlines()
...
def get_synonym_dict():
    all_poses = set()
    with open('../../../data/sws_ds/thesaurus.json', 'r') as f:
...
def randomChange(sent, substitutedict):
    ...
    for i, word in enumerate(words):
        ...
        if ' '.join(words[i:i+3]) not in set(stopwords.words('english')) and substitutedict[' '.join(words[i:i+3])] and 'phrase' in substitutedict[' '.join(words[i:i+3])] and substitutedict[' '.join(words[i:i+3])]['phrase']:
            ...
            if subthis: 
                targets.append((random.choice(substitutedict[' '.join(words[i:i+3])]['phrase']), ' '.join(words[i:i+3]), i, 3))
                toskip = 2
            ...
            if subthis:
                targets.append((random.choice(substitutedict[' '.join(words[i:i+2])]['phrase']), ' '.join(words[i:i+2]), i, 2))
                toskip = 1
            ...
            if candidate_list: 
                ...
                targets.append((candidate_list, word, i, 1))
    ...
    for i in range(len(targets)):
        if i in targets_idx:
            ...
            res.append([[targets[i][1], targets[i][2], targets[i][2]+targets[i][3]], targets[i][0] + ['none']*(10-len( targets[i][0]))])
...
for k, v in tqdm(j.items()):
    res[k] = {}
    res[k]["input_words"] = v['sentence']
    res[k]["substitute_topk"] = randomChange(v['sentence'], substitutedict)[2]
f = open('res_random.json', 'w')
